...
공부할 때 탭이 몇 개나 열려 있나요? 옵시디언으로 노트를 쓰고, 블로그에 정리하고, 구글링으로 자료를 찾는 루틴을 반복하다 보면 컨텍스트는 끊기고 정보는 흩어져 있습니다.

NewLearn Note는 이 흐름을 하나로 묶습니다. GitHub처럼 **Library**에 노트를 push/pull하고, **Publish** 하나로 즉시 공개됩니다. 다른 사용자가 내 노트를 참조하면 노트 간 네트워크가 형성됩니다. 개인의 학습 기록이 집단지성으로 이어지는 구조입니다.

AI 문서 분석 기능은 별도 프로토타입 [Nura](https://1dohyeon.github.io/projects/nura)를 독립 개발해 RAG 파이프라인을 먼저 검증했습니다. 메인 프로젝트에 바로 통합하기보다 각 단계의 문제를 먼저 측정하고 싶었습니다.
...
NewLearn Note에 AI 문서 분석 기능을 도입하기 전, 메인 프로젝트에 바로 통합하기보다 독립 환경에서 먼저 검증하고 싶었습니다. RAG 파이프라인은 처음 다뤄보는 영역이라, 청킹 전략이나 프롬프트 설계를 실험할 별도 공간이 필요했습니다.

그래서 [Nura](https://1dohyeon.github.io/projects/nura)를 FastAPI 기반 독립 프로토타입으로 개발해 파이프라인의 각 단계를 검증했습니다.

### 검증한 핵심 문제들

**NestJS 대신 FastAPI를 선택한 이유**

LangChain, ChromaDB, OpenAI SDK가 모두 Python 기반이라, NestJS에서 연동하려면 별도 Python 프로세스를 띄우거나 외부 API로 감싸야 했습니다. 프로토타입 목적인데 불필요한 복잡도를 더하고 싶지 않아 FastAPI로 Python AI 생태계를 네이티브로 활용했습니다.

**비동기 처리 - 30초 대기 문제**

100페이지 논문 PDF를 업로드하면 파싱과 임베딩 생성에 약 30초가 걸렸습니다. FastAPI의 `BackgroundTasks`를 활용해 파일이 들어오면 메타데이터만 저장하고 즉시 응답을 반환한 뒤, 실제 처리는 백그라운드에서 수행하도록 변경했습니다.

**지능형 프롬프트 분기 - LLM 환각 방지**

문서와 무관한 질문에도 LLM이 업로드된 문서를 억지로 끼워 맞춰 답변하는 문제가 있었습니다. 관련 청크가 검색된 경우와 그렇지 않은 경우를 분기해, 연관성이 낮을 때는 일반 지식으로 답변하도록 프롬프트를 설계하여 해결했습니다.
