...
- **엔드포인트 은닉을 위한 Reverse Proxy 및 BFF 패턴 적용**
  - 핵심 비즈니스 로직이 담긴 API 서버의 엔드포인트를 외부에 직접 노출하지 않고, Next.js(Web) 서버를 리버스 프록시 레이어로 배치하여 보안을 강화했습니다.
  - 클라이언트가 여러 마이크로서비스와 직접 통신하는 대신 단일 접점(BFF)을 거치게 하여 CORS 설정을 단순화하고, 서버 측에서 데이터 전처리 및 SSR을 수행하여 일관된 데이터 무결성을 유지합니다.

- **인증(Auth) 흐름의 신뢰성을 위한 Direct Path 예외 처리**
  - 모든 요청을 Web 서버로 은닉하는 원칙을 유지하되, Google OAuth와 같은 인증 흐름은 API 서버로 직접 라우팅되도록 예외 경로를 설계했습니다.
  - 이는 소셜 로그인 리다이렉트 과정에서 발생할 수 있는 프록시 오버헤드와 세션 유실을 방지하기 위함이며, API 서버가 발행하는 `HttpOnly` 쿠키가 브라우저에 정확히 안착할 수 있도록 통신 경로를 최적화한 결과입니다.

- **Cloudflare Workers를 활용한 Edge 단의 헤더 최적화 (RFC 6265 준수)**
  - GCP Cloud Run의 기본 도메인(`.run.app`) 환경에서 발생하는 멀티 도메인 간 쿠키 공유 문제를 해결하기 위해 Cloudflare Workers를 채택했습니다.
  - 일부 프록시 환경에서 다중 `Set-Cookie` 헤더를 쉼표(`,`)로 병합하여 브라우저가 인식하지 못하는 표준 위반 사례를 방지하고자, 엣지 단에서 `getSetCookie()` API를 통해 헤더를 RFC 6265 표준에 맞게 재가공하는 로직을 구현했습니다. 이를 통해 인프라 비용 추가 없이 인증 문제를 안정적으로 해결했습니다.

- **Serverless 기반의 확장성 및 비용 효율성 확보**
  - 모든 애플리케이션 서비스(Next.js, NestJS, FastAPI)를 Docker 컨테이너화하여 GCP Cloud Run에 배포함으로써 트래픽에 따른 자동 확장(Auto-scaling)과 운영 비용 최적화를 달성했습니다.
  - 인프라 관리에 드는 리소스를 최소화하고, 비즈니스 로직과 데이터 파이프라인 고도화에 집중할 수 있는 환경을 구축했습니다.
