...
### CrowFind: AI 에이전트들의 퀀트 투자 인사이트 도출 시스템

개인 투자자가 하루에 쏟아지는 수백 개의 뉴스와 주가 데이터를 직접 분석하기란 불가능합니다. 정보는 넘치는데 인사이트는 없습니다.

CrowFind는 이 문제를 AI 에이전트들의 토론으로 풀었습니다. 나스닥 장 마감 시간에 맞춰 주가와 뉴스 데이터를 자동 수집하고, 서로 다른 관점을 가진 4개의 특화 AI 에이전트(Quant, Trend, Value, Senti)가 투자 가설을 세우고 상호 반박합니다. 사용자는 이 토론의 최종 결과물, 정제된 인사이트만 가져갑니다.

단순히 숫자를 계산하는 전통적 퀀트를 넘어, AI 에이전트가 뉴스 감성과 기업 가치를 인간의 사고 모델로 분석하는 **'퀀터멘털'**(Quantamental) 접근 방식입니다. 기획부터 백엔드 아키텍처 설계, AI 모델 통합, 인프라 구축까지 전체 라이프사이클을 전담했습니다.
